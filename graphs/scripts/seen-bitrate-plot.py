...
for i in range(N_RUN):
    bitrate_ncn = pd.concat([bitrate_ncn, read_bitrate ( '../../results/results_4/' + str(i) + '/netcodndn/dash-trace.txt' )], axis=1, ignore_index=True)
    bitrate_ndn = pd.concat([bitrate_ndn, read_bitrate ( '../../results/results_4/' + str(i) + '/ndn/dash-trace.txt' )], axis=1, ignore_index=True)
